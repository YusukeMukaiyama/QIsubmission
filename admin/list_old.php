...
function enum_enquete ()
{
	...
	if (isset($_REQUEST['edit']) && $_REQUEST['edit']) {
		...
		while ( $fld = mysqli_fetch_object ( $rs ) ) {
			...
			if ( $fld->type == Config::TEXT || $fld->type == Config::TEXTAREA || $fld->type == Config::RADIO ) {
	...
	if ( $id == Config::OUTCOME ) {	// アウトカム
		...
		while ($fld = mysqli_fetch_object ( $rs )) {
			...
			if (isset($qfld) && $qfld !== null && $qfld->qtype == Config::TEXT) {
		...
		while ( $fld = mysqli_fetch_object ( $rs ) ) {	// 質問・回答の表示
			...
			while ( $qfld = mysqli_fetch_object ( $qrs ) ) {
				...
				echo "<tr><td>";
				if ( $qfld->qtype == Config::TEXT ) {	// 回答形式がテキスト入力タイプの場合
