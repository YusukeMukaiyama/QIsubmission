...
$years = $db->query("SELECT DISTINCT LEFT(uid, 2) AS itemdata FROM usr ORDER BY itemdata DESC")->fetch_all(MYSQLI_ASSOC);
$prefs = [];
$hospitals = [];
$wards = [];

if (!empty($_POST['year'])) {
    $stmt = $db->prepare("SELECT DISTINCT SUBSTRING(uid, 4, 2) AS itemdata FROM usr WHERE uid LIKE ? ORDER BY itemdata ASC");
    $yearTerm = $_POST['year'] . '-%';
    $stmt->bind_param("s", $yearTerm);
    $stmt->execute();
    $prefs = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
}

if (!empty($_POST['year']) && !empty($_POST['pref'])) {
    $stmt = $db->prepare("SELECT DISTINCT SUBSTRING(uid, 7, 4) AS itemdata FROM usr WHERE uid LIKE ? ORDER BY itemdata ASC");
    $prefTerm = $_POST['year'] . '-' . $_POST['pref'] . '-%';
    $stmt->bind_param("s", $prefTerm);
    $stmt->execute();
    $hospitals = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
}

if (!empty($_POST['year']) && !empty($_POST['pref']) && !empty($_POST['hosp'])) {
    $stmt = $db->prepare("SELECT DISTINCT SUBSTRING(uid, 12, 2) AS itemdata FROM usr WHERE uid LIKE ? ORDER BY itemdata ASC");
    $wardTerm = $_POST['year'] . '-' . $_POST['pref'] . '-' . $_POST['hosp'] . '-%';
    $stmt->bind_param("s", $wardTerm);
    $stmt->execute();
