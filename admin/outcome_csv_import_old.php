...
uppdate_point
  ...
  戻値：なし
-------------------------------------------------------------------------------------------------*/
function update_point ( $year, $id1 ,$id2 ,$id3 ,$id4 )
{
  global $db;
  $sql = "SELECT ans_id,point FROM ans WHERE id=3 AND id1=".$id1." AND id2=".$id2." AND id3=".$id3." AND id4=".$id4." ORDER BY ans_id";
  $rs = mysqli_query ( $db ,$sql );
  while ( $fld = mysqli_fetch_object ( $rs ) ) {
    $sql = "UPDATE usr_ans SET point=".$fld->point." WHERE id=3 AND id1=".$id1." AND id2=".$id2." AND id3=".$id3." AND id4=".$id4." AND ans=".$fld->ans_id." AND uid LIKE '".$year."%'";
print $sql."\n";
    //mysqli_unbuffered_query ( $db , $sql ) or die ( "データインポートエラー LINE:".__LINE__."(".$sql.")" );
    mysqli_query($db, $sql, MYSQLI_USE_RESULT) or die("データインポートエラー LINE:".__LINE__."(".$sql.")");
...
function import_csv()
{
  ...
  //if ( sizeof ( $header ) != 22 ) die ( "データの形式が不正です。" );
...
// 5  ID4
  ...
  while ( ( $data = fgetcsv ( $hFile ,1024 ,"," ) ) !== FALSE ) {
    ...
    // ----------------------------------------------------------------------------------------
    // アウトカム(051〜999の範囲の最大値 + 1)
    $uid = sprintf("%02d-%02d-%04d-%02d",$data[2],$data[3],$data[4],$data[5]);
    $sql = "SELECT uid FROM usr WHERE id = '3' AND uid > '".$uid."-050' AND uid LIKE '".$uid."-%' ORDER BY uid DESC LIMIT 1";
    $rs = mysqli_query ( $db ,$sql );
    ...
    // ----------------------------------------------------------------------------------------
    // usrインポート処理
    $sql = "INSERT INTO usr(id,uid,pass,comp,del,lastupdate) VALUES(3,'".$uid."','".simplepasswd()."','1','1','".date( 'Y-m-d' )."')";
    ...
    // enq_usr_ansインポート処理
    // 6  性別
    $sql = "INSERT INTO enq_usr_ans(id,id1,uid,ans) VALUES(30,1,'".$uid."','".$data[7]."')";
    //mysqli_unbuffered_query ( $sql ,$db ) or die ( "データインポートエラー LINE:".__LINE__."(".$sql.")" );
    mysqli_query($db, $sql, MYSQLI_USE_RESULT) or die("データインポートエラー LINE:".__LINE__."(".$sql.")");

    // 7  年齢
    $sql = "INSERT INTO enq_usr_ans(id,id1,uid,ans) VALUES(30,2,'".$uid."','".$data[6]."')";
    //mysqli_unbuffered_query ( $sql ,$db ) or die ( "データインポートエラー LINE:".__LINE__."(".$sql.")" );
    mysqli_query($db, $sql, MYSQLI_USE_RESULT) or die("データインポートエラー LINE:".__LINE__."(".$sql.")");

    // 8  入院期間
    $sql = "INSERT INTO enq_usr_ans(id,id1,uid,ans) VALUES(30,3,'".$uid."','".$data[8]."')";
    //mysqli_unbuffered_query ( $sql ,$db ) or die ( "データインポートエラー LINE:".__LINE__."(".$sql.")" );
    mysqli_query($db, $sql, MYSQLI_USE_RESULT) or die("データインポートエラー LINE:".__LINE__."(".$sql.")");
    ...
    // ----------------------------------------------------------------------------------------
    // usr_ansインポート処理（全てのpointを一旦0に設定）
    $i = 9;
    foreach($header_ids as $key => $val){
      if(isset($header_id_val[$val][$data[$i]]) ){
        $sql = "INSERT INTO usr_ans(id,uid,id1,id2,id3,id4,ans,point) VALUES(3,'".$uid."',".$val[0].",".$val[1].",".$val[2].",".$val[3].",".$header_id_val[$val][$data[$i]].",0)";
        //mysqli_unbuffered_query ( $sql ,$db ) or die ( "データインポートエラー LINE:".__LINE__."(".$sql.")" );
        mysqli_query($db, $sql, MYSQLI_USE_RESULT) or die("データインポートエラー LINE:".__LINE__."(".$sql.")");

      }else{
        $sql = "INSERT INTO usr_ans(id,uid,id1,id2,id3,id4,ans,point) VALUES(3,'".$uid."',".$val[0].",".$val[1].",".$val[2].",".$val[3].",'',0)";
        //mysqli_unbuffered_query ( $sql ,$db ) or die ( "データインポートエラー LINE:".__LINE__."(".$sql.")" );
        mysqli_query($db, $sql, MYSQLI_USE_RESULT) or die("データインポートエラー LINE:".__LINE__."(".$sql.")");
...
//  if ( $cnt ) {
    foreach($header_id as $key => $val){
      update_point ( $year, $val[0], $val[1], $val[2], $val[3] );
    }
    /*
    // 9  1111
...
//  }
  ...
  ///////////////////////////////////////////////////////////////////////////////////////
  //元々if (is_uploaded_file($_FILES['userfile']['tmp_name'])) {これで、無理やりエラーなくしてるから後で修正
