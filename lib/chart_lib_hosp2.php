...
class PDF_Hosp extends PDF_Japanese
{
	...
	*******************************************************************/
	...
	/*******************************************************************
		...
		戻値：なし
	*******************************************************************/
	function SaveChart ( $hosp, $filepath = "./pdf_bat/" )
	{
		// ファイル名
		$filename = $hosp."_total.pdf";

		// PDF作成
		$this->CreateChart ( $hosp );
	...
	*******************************************************************/
	function CreateChart ( $ID )
	{
		...
		*******************************************************************/
		...
		$type_str = 1;
		// カテゴリ文字列取得 ( 構造 )
		$category_str_str = getCategoryStr ( $type_str );
		// 大項目 ( 構造 )
		...
		// 評価 ( 構造 )
		$point_array_str = getAllPastEvaluationHospital ( $ID, $year, $type_str );
		...
		$sql = "SELECT (CASE id1 WHEN '9' THEN '内科' WHEN '10' THEN '外科' WHEN '11' THEN '産婦人科' WHEN '12' THEN '小児科' ".
				...
				"WHERE id = 11 AND id1 IN (9,10,11,12,13,14) AND ans = 1 AND uid LIKE '".$ID."%' GROUP BY id1";
		...
		// 稼動病床数：全体　999　一般　999
		$str = "稼動病床数：";
		$sql = "SELECT ans FROM enq_usr_ans WHERE id = 11 AND id1 = 3 AND uid LIKE '".$ID."%' ORDER BY ans DESC LIMIT 1";
		$rs =  mysqli_query ( $db , $sql  );
		if ( $fld = mysqli_fetch_object ( $rs ) ) {
			$str .= "全体　".$fld->ans."　";
		}
		$sql = "SELECT ans FROM enq_usr_ans WHERE id = 11 AND id1 = 4 AND uid LIKE '".$ID."%' ORDER BY ans DESC LIMIT 1";
		$rs =  mysqli_query ( $db , $sql  );
		if ( $fld = mysqli_fetch_object ( $rs ) ) {
			$str .= "一般　".$fld->ans."　";
		}
...
/*******************************************************************
	病棟単位の前回/前々回の評価を取得します。
	概要：病棟単位の前回/前々回の評価を取得
	引数：ID
	戻値：病棟単位の前回/前々回の評価
*******************************************************************/
function getAllPastEvaluationHospital ( $hosp_id, $arg_year, $category )
{
	...
	$tmp_id = substr ( $hosp_id, 2 );	// 年度以外のID部
	...
	$rs =  mysqli_query ( $db , $sql  );
	if ( !mysqli_num_rows ( $rs ) ) return NULL;	// データなし
	while ( $fld = mysqli_fetch_object ( $rs ) ) {
	...
	for ( $i = 0;$i < 3;$i++ ) {
		...
		$serch_id = sprintf ( "%02d%s", $year[$i], $tmp_id );	// 検索用ID

		// --------------------------------------------------------------------------------------------
		// 平均の取得（小数点第一位まで表示）
		// --------------------------------------------------------------------------------------------
		// 集計一時テーブルの初期化
		//mysql_unbuffered_query ( "TRUNCATE TABLE ans_total" ,$db );
...
/*
		$sql = "INSERT INTO ans_total(id,id1,uid,point) ".
			...
			"item4.qtype = '1' AND usr.comp = '1' AND usr.del = '1' AND usr.uid LIKE '".$year[$i]."%' ".
...
/*
		$sql = "INSERT INTO ans_total(id,id1,uid,point) ".
			...
			" AND usr.comp = '1' AND usr.del = '1' AND usr.uid LIKE '".$serch_id."%'  ".
			"GROUP BY usr_ans.id,usr_ans.id1,usr_ans.uid ";
*/
		// ADD END
    ...
    // 設問内容を変更した際に過去の点数が下がってしまう為、設問の紐づき関係なく単純集計に変更。
		$sql = "INSERT INTO ans_total(id,id1,uid,point) "
			 ...
			 . " AND usr.uid LIKE '".$serch_id."%'  "
			 . "GROUP BY usr_ans.id,usr_ans.id1,usr_ans.uid ";
		// ADD END
//error_log ( $sql."\n" , 3, "/home/hyougo2/public_html/lib/debug.log" );
		//mysql_unbuffered_query ( $sql ,$db );
		// 集計一時テーブルの初期化(非バッファクエリ)
    ...
    if ($result) {
		...
		// 大項目ごとに評価を取得
		for ( $j = 0;$j < count ( $title_id_array );$j++ ) {

			$id1 = ( int )$title_id_array[$j];	// 大項目ID

			$sql = "SELECT ROUND(AVG(point),1)AS avg_point FROM ans_total WHERE id = '".$category."' AND id1='".$id1."' AND uid LIKE '".$serch_id."%'";
//error_log ( $sql."\n" , 3, "/home/hyougo2/public_html/lib/debug.log" );
			$rs =  mysqli_query ( $db , $sql  );
...
function getEncAverage ( $year )
{
	...
	$average_array = array ( );	// 平均格納

	// 転倒、転落、ジョクソウ、院内感染、誤薬を固定で取得
	for ( $i = 1;$i <= 5;$i++ ) {
		// 人数取得
		$sql = "SELECT COUNT(usr.uid)AS cnt FROM enq_usr_ans, usr ".
			"WHERE enq_usr_ans.uid=usr.uid AND usr.uid LIKE '".$year."%' AND usr.comp='1' AND usr.del='1' AND enq_usr_ans.id='10' AND enq_usr_ans.id1='".$i."'";
		$rs =  mysqli_query ( $db , $sql  );
		$fld = mysqli_fetch_object ( $rs );
		$cnt_enq = $fld->cnt;
		if ( $cnt_enq ) {	// 平均点取得
			$sql = "SELECT SUM(ans)AS sum_ans FROM enq_usr_ans, usr ".
				"WHERE enq_usr_ans.uid=usr.uid AND usr.uid LIKE '".$year."%' AND usr.comp='1' AND usr.del='1' AND enq_usr_ans.id='10' AND enq_usr_ans.id1='".$i."'";
			$rs =  mysqli_query ( $db , $sql  );
			$fld = mysqli_fetch_object ( $rs );
			$sum_ans = $fld->sum_ans;
			if ( $sum_ans ) {
				$average_array[] = round ( $sum_ans / $cnt_enq, 2 );
			} else {
				$average_array[] = 0;
			}
		} else {
...
function getEncResult ( $year, $ID )
{
	...
	$enq_arr = array ( );	// 結果格納配列
	$iyear = ( int )$year;	// 年度 ( int )
	$tmp_id = substr ( $ID, 2 );	// 年度以外のID部

	for ( $i = 0;$i < 3;$i++, $iyear-- ) {	// 今年度から 3年分取得

		$tmp_arr = array ( );	// 一時格納配列
		$serch_id = sprintf ( "%02d%s", $iyear, $tmp_id );	//検索用ID

		// 転倒、転落、ジョクソウ、院内感染、誤薬を固定で取得
		for ( $j = 1;$j <= 5;$j++ ) {

			// 人数取得 ( 病院 )
			$sql = "SELECT COUNT(u.uid)AS cnt FROM enq_usr_ans e, usr u ".
					"WHERE e.uid=u.uid AND u.uid LIKE '".$serch_id."%' AND u.comp='1' AND u.del='1' AND e.id='10' AND e.id1='".$j."'";
			$rs =  mysqli_query ( $db , $sql  );
			$fld = mysqli_fetch_object ( $rs );
			$cnt_enq = $fld->cnt;

			if ( $cnt_enq ) {	// 平均点取得
				$sql = "SELECT SUM(ans)AS sum_ans FROM enq_usr_ans e, usr u ".
						"WHERE e.uid=u.uid AND u.uid LIKE '".$serch_id."%' AND u.comp='1' AND u.del='1' AND e.id='10' AND e.id1='".$j."'";
				$rs =  mysqli_query ( $db , $sql  );
				$fld = mysqli_fetch_object ( $rs );
				$sum_ans = $fld->sum_ans;
				if ( $sum_ans ) {
					$tmp_arr[] = round ( $sum_ans / $cnt_enq, 2 );
				} else {
					$tmp_arr[] = 0;
				}
			} else {
