...
class PDF extends TCPDF
{
  ...
  public function __construct() {
    ...
    if (file_exists($fontfile)) {
        ...
        if ($fontname === false) {
            error_log("TCPDFにフォントを追加する際にエラーが発生しました: $fontfile", 3, $log_file);
            exit("TCPDFにフォントを追加する際にエラーが発生しました。\n");
        } else {
            error_log("TCPDFにフォントを追加しました: $fontfile", 3, $log_file);
        }
    } else {
        error_log("フォントファイルが見つかりません: $fontfile", 3, $log_file);
        exit("フォントファイルが見つかりません: $fontfile\n");
    }
  ...
  *******************************************************************/
  public function CreateChart ( $id, $arr_avg,$view = TRUE )
  {
    ...
    $this->SetFont('zenoldmincho', '', 12);  // ここでフォントを設定
    // マージンを設定
