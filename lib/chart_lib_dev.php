...
//require_once("../lib/tfpdf/japanese.php");
require_once("../lib/tfpdf/tdraw.php");
// tFPDFライブラリを読み込む前に定数を定義
define('_SYSTEM_TTFONTS', '/Users/mukaiyamayusuke/Desktop/Sites/QIsystem/kango3/public_html/lib/tfpdf/font/');
...
class PDF extends PDF_Draw
{
  ...
  // 平均/評価の文字列
  //var $str_point = array ( "全国平均", "今回の結果", "前回の結果", "前々回の結果" );
  var $str_point = array("全国平均", "今年の結果", "前年の結果" ,"前々年の結果");
  ...
  // セルの高さ ( ヘッダ )
  var $cell_header_height = 8;
  // セルの高さ ( データ )
  var $cell_data_height = 8;
  // セルの幅 ( 項目 )
  var $cell_title_width = 50;
  // セルの幅 ( 点数 )
  var $cell_point_width = 30;
  ...
  var $base_line_color = 192;
  // 外周の多重度
  var $round_count = 4;
  // パーセント表示のフォントサイズ
  var $percent_font_size = 10;
  // 項目タイトルのフォントサイズ
  var $item_font_size = 14;

  // データ線色 ( R, G, B ) 全国の平均−今回の評価−前回の評価−前々回の評価
  var $data_line_color = array ( array ( 192, 0, 0 ), array ( 0, 192, 0 ), array ( 0, 0, 192 ), array ( 192, 0, 192 ) );
  // データ線の太さ
  var $data_line_width = 0.5;
  // 点線の実線部長の配列
  var $dash_array = array ( array ( 5 ), array ( ), array ( 10, 3 ), array ( 10, 3, 3 ) );
  // 点線の空白長
  var $dash_space_length = 5;


  /*ヒイラギ丸ゴシックの時、ただjapanese.pdfはSJISしかサポートしてないっぽい
  */
  function __construct() {
    parent::__construct(); // 親クラスのコンストラクタを呼び出す
    $this->AddPage();
    // 日本語フォントの追加
    $this->AddFont('HiraMaruPro', '', 'HiraMaruProN-W4-AlphaNum-02.ttf', true);
    $this->SetFont('HiraMaruPro', '', 12);
  }
  ...
  *******************************************************************/
  function CreateChart ( $id, $arr_avg,$view = TRUE )
  {
    ...
    // 既存のフォントを追加
    ////////////////////////////////////////////////////////////既存のフォントを追加する部分を省略__constructで記述////////////////////////////////////////////////////////////
    //$this->AddSJISFont();
    //$this->AddSJIShwFont();
    ...
    // セルのタイトル表示
    //$str = mb_convert_encoding ( $category_str."得点結果　", "SJIS", "UTF-8" );
    //$str = mb_convert_encoding ( $category_str."得点結果　", "SJIS", "EUC-JP" );
    ...
    // フォントサイズの設定
    $this->SetFontSize ( 10 );
    //$str = mb_convert_encoding ( "注）全国平均とは".( $view ? "昨" : "今" )."年度参加した全国の病棟の平均値です。", "SJIS", "EUC-JP" );
    //$str = mb_convert_encoding ( "注）全国平均とは".( $view ? "昨" : "今" )."年度参加した全国の病棟の平均値です。", "SJIS", "UTF-8" );
    // 変更後
    ...
    $this->SetFontSize ( $this->title_font_size );
    // レーダーチャートのタイトル表示
    //$str = mb_convert_encoding ( $category_str."得点レーダー", "SJIS", "EUC-JP" );
    //$str = mb_convert_encoding ( $category_str."得点レーダー", "SJIS", "UTF-8" );
    ...
    $this->SetFontSize ( 14 );
    // セルのタイトル表示
    //$str = mb_convert_encoding ( "当該領域で「回答しない」が１項目以上あった場合は0点として表示されます。", "SJIS", "EUC-JP" );
    //$str = mb_convert_encoding ( "当該領域で「回答しない」が１項目以上あった場合は0点として表示されます。", "SJIS", "UTF-8" );
    $str = "当該領域で「回答しない」が１項目以上あった場合は0点として表示されます。";

    
    $this->Write ( 8, $str );
    $this->Ln ( );
    //$str = mb_convert_encoding ( "※満点を100として%で表示しております。", "SJIS", "EUC-JP" );
    //$str = mb_convert_encoding ( "※満点を100として%で表示しております。", "SJIS", "UTF-8" );
  ...
  *******************************************************************/
  function createCell ( $title_array, $arr_pt, $max_array )
  {
    ...
    // セルのヘッダ部を描画 ※項目（満点）
    //$this->Cell ( $this->cell_title_width, $this->cell_header_height, mb_convert_encoding ( "項目（満点）", "SJIS", "EUC-JP" ), 1, 0, "C" );
    //$this->Cell ( $this->cell_title_width, $this->cell_header_height, mb_convert_encoding ( "項目（満点）", "SJIS", "UTF-8" ), 1, 0, "C" );
    ...
    for ( $i = 0;$i < count ( $this->str_point );$i++ ) {
      //$this->Cell ( $this->cell_point_width, $this->cell_header_height, mb_convert_encoding ( $this->str_point[$i], "SJIS", "EUC-JP" ), 1, 0, "C" );
      //$this->Cell ( $this->cell_point_width, $this->cell_header_height, mb_convert_encoding ( $this->str_point[$i], "SJIS", "UTF-8" ), 1, 0, "C" );
    ...
    for ( $i = 0;$i < $this->top_count;$i++ ) {
      ...
      //$this->MultiCell ( $this->cell_title_width, $this->cell_data_height, mb_convert_encoding ( $tmp, "SJIS", "EUC-JP" ), 1, "L" );
      //$this->MultiCell ( $this->cell_title_width, $this->cell_data_height, mb_convert_encoding ( $tmp, "SJIS", "UTF-8" ), 1, "L" );
  ...
  *******************************************************************/
  function CreateChartBase ( $point_count, $max_chart )
  {
    ...
    for ( $j = 0;$j <= $this->round_count;$j++ ) {
      ...
      // 文字列設定
      //$str = mb_convert_encoding ( ( $interbal_data * $j ), "SJIS", "EUC-JP" );
      //$str = mb_convert_encoding ( ( $interbal_data * $j ), "SJIS", "UTF-8" );
  ...
  *******************************************************************/
  function CreateRaderChartTitle ( $title_array )
  {
    ...
    for ( $i = 0;$i < $title_count;$i++ ) {
      ...
      // 文字書き込み
      //$this->Write ( 0, mb_convert_encoding ( $title_array[$i], "SJIS", "EUC-JP" ) );
      //$this->Write ( 0, mb_convert_encoding ( $title_array[$i], "SJIS", "UTF-8" ) );
  ...
  *******************************************************************/
  function createLineDetails ( $arr_pt )
  {
    ...
    // 長方形をセルで表示
    //$this->MultiCell ( $cell_details_width, $cell_details_height, mb_convert_encoding ( $str, "SJIS", "EUC-JP" ), 1, "R" );
    //$this->MultiCell ( $cell_details_width, $cell_details_height, mb_convert_encoding ( $str, "SJIS", "UTF-8" ), 1, "R" );
