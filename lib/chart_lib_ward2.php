...
class PDF_Ward extends TCPDF {
  ...
  *******************************************************************/
  function CreateChart ( $ID )
  {
    ...
    *******************************************************************/
    ...
    $this->SetFontSize ( 14 );
    // セルのタイトル表示
    $str = mb_convert_encoding ( "当該領域で「回答しない」が１項目以上あった場合は0点として表示されます。", "hminchomedium", "EUC-JP" );
    $this->Write ( 8, $str );
    $this->Ln ( );
    $str = mb_convert_encoding ( "※満点を100として%で表示しております。", "hminchomedium", "EUC-JP" );
    $this->Write ( 8, $str );
  ...
  *******************************************************************/
  function createCell ( $title_array, $point_array, $max_array )
  {
    ...
    // セルのヘッダ部を描画 ※項目（満点）
    $this->Cell ( $this->cell_title_width, $this->cell_header_height, mb_convert_encoding ( "項目（満点）", "hminchomedium", "EUC-JP" ), 1, 0, "C" );

    for ( $i = 0;$i < count ( $this->str_point );$i++ ) {
      $this->Cell ( $this->cell_point_width, $this->cell_header_height, mb_convert_encoding ( $this->str_point[$i], "hminchomedium", "EUC-JP" ), 1, 0, "C" );
    }
    $this->Ln ( );
    ...
    for ( $i = 0;$i < $this->top_count;$i++ ) {
      ...
      $this->MultiCell ( $this->cell_title_width, $this->cell_data_height, mb_convert_encoding ( $tmp, "hminchomedium", "EUC-JP" ), 1, "L" );
  ...
  *******************************************************************/
  function createCellOutCome($enq_result)
  {
    ...
    for ( $i = 0;$i < 5;$i++ ) {
      ...
      $this->MultiCell ( $this->cell_title_width ,$this->cell_data_height ,mb_convert_encoding ( $enq_title ,"hminchomedium" ,"EUC-JP" ), 1, 'L');
  ...
  *******************************************************************/
  function CreateChartBase ( $point_count, $max_chart )
  {
    ...
    for ( $j = 0;$j <= $this->round_count;$j++ ) {
      ...
      // 文字列設定
      $str = mb_convert_encoding ( ( $interbal_data * $j ), "hminchomedium", "EUC-JP" );
      // 座標設定
  ...
  *******************************************************************/
  function CreateRaderChartTitle ( $title_array )
  {
    ...
    for ( $i = 0;$i < $title_count;$i++ ) {
      ...
      // 文字書き込み
      $this->Write ( 0, mb_convert_encoding ( $title_array[$i], "hminchomedium", "EUC-JP" ) );
    }
  }
  ...
  *******************************************************************/
  function createLineDetails ( $point_array )
  {
    ...
    // 長方形をセルで表示
    $this->MultiCell ( $cell_details_width, $cell_details_height, mb_convert_encoding ( $str, "hminchomedium", "EUC-JP" ), 1, "R" );
