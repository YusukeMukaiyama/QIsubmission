...
class PDF_Japanese extends PDF_Draw
{
function AddCIDFont($family,$style,$name,$cw,$CMap,$registry)
{
	// $this->fonts 配列が未定義の場合に初期化
    if (!isset($this->fonts)) {
        $this->fonts = array();
    }
	...
	// $this->fontsが配列であることを保証する//////////////////////////付け加えた部分
...
function _putfonts()
{
	...
	foreach($this->FontFiles as $file => $info) {
		...
		$size = filesize($file);
		if(!$size) $this->Error('Font file not found: ' . $file); // エラーメッセージにファイル名を含める
		// 以降のコード...
	...
	foreach($this->fonts as $k=>$font) {
		...
		if($font['type'] == 'Type0') {
			$this->_putType0($font); // ここで完全なフォントオブジェクトを出力
		} else {
			$name=$font['name'];
